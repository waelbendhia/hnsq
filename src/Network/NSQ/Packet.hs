...
readPackets bytes =
  case readPacket bytes of
    (Nothing, _) -> ([], bytes)
    (Just msg, bytes') ->
      let (next, bytes'') = readPackets bytes'
      in ([msg] ++ next, bytes'')

readPacket :: B.ByteString -> (Maybe Packet, B.ByteString)
readPacket bytes =
  case runGetOrFail get (BL.fromStrict bytes) of
...
readFrame size frame
  ...
  | otherwise = MessagePacket <$> (Message <$> getInt64be <*> getInt16be <*> getByteString 16 <*> getByteString (fromIntegral size - 30))
