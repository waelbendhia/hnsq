...
mpub topic payload =
  "MPUB " <> topic <> "\n" <> toStrict (encode totalSize) <> B.concat (fmap message payload)
  where
    len bytes = (fromIntegral $ B.length bytes) :: Int32
    totalSize = Prelude.foldl (+) 0 $ fmap len payload
...
req msgId = "REQ " <> msgId <> " " <> toStrict (encode (1000 :: Int32))
